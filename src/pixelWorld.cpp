...
PixelWorld::PixelWorld(std::shared_ptr<SDL_Renderer> renderer, int window_width, int window_height, int width, int height, float scale) {
  m_x = window_width / 2 - (width*scale) / 2;
  m_y = window_height / 2 - (height*scale) / 2;
  m_width = width;
  m_height = height;
  m_scale = scale;
